...
export const getAllUsers = async (req, res) => {
  try {
    const users = await UserModel.find({})
      .select("name email isAllowedToDeposit isAllowedToWithdraw createdAt")
      .sort({ createdAt: -1 })
      .lean();
...
export const updateUserPermission = async (req, res) => {
  try {
    ...
    if (type === "deposit") {
      ...
      update.isAllowedToWithdraw = allowed;
      update.blockedWithdrawalReason = allowed ? null : reason || null;
    }
